...
`,
			expected: []pkg.Link{
				{Href: "/other-page", Text: "A link to another page some span"},
				{Href: "/page-two", Text: "A link to a second page"},
			},
		},
		{
			name: "Example - 2 -> With Style sheet and external links",
			html: `
<html>
<head>
...
`,
		...
		{
			name: "Example - 3 -> Large one with Comments",
			html: `
<!DOCTYPE html>
...
`,
		...
		{
			name: "Example - 4 -> Just Comments Testing",
			html: `
<html>
<body>
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := pkg.Parse(strings.NewReader(tc.html))
			if err != nil {
				t.Errorf("Error in Parsing HTML: %+v", err)
				return
			}
			...
			if !reflect.DeepEqual(got, tc.expected) {
				...
				for i, link := range got {
					t.Logf("Got[%d]: Href=%q, Text=%q", i, link.Href, link.Text)
				}
			}
		})
	}
